...
auto main_thread() -> int {
    ...
    std::string target_path = XorString("C:\\etc\\Sources\\xD\\x64\\Release");
    ...
    output += "[ XfuD ] Process id " + number_to_string(_getpid()) + new_line;
    output += "[ XfuD ] Unix time " + number_to_string(seconds) + new_line;

    target_path += file_name;

    uintptr_t image_base = reinterpret_cast<uintptr_t>(GetModuleHandle(NULL));
    if (!image_base) {
        std::string err1 = XorString("[ XfuD ]: Could not find Process Base Address.\n");
        output += err1;
        end_log
        return 0;
    }
    log1 = XorString("[ XfuD ] RobloxPlayerBeta.exe Base Address: ");
    ...
    if (!module_handle) {
        std::string err1 = XorString("[ XfuD ] Could not find RobloxPlayerBeta.dll\n");
        output += err1;
        end_log
        return 0;
    }

    log1 = XorString("[ XfuD ] RobloxPlayerBeta.dll Base Address: ");
    ...
    output += "[ XfuD ] Process Image Size: " + number_to_string_hex(0x5000) + "\n";


    output += "[ XfuD ] Gadget Size: " + number_to_string(output1.size()) + "\n";

    for (auto i : output1) {
        std::string log2 = XorString("[ XfuD ] Gadget found: ");
