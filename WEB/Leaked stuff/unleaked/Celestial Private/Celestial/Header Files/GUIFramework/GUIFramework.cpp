...
namespace ImGuiInternals
{
	...
	bool SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
	{
		...
		//const ImRect hoveredRect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(window->Size.x - g.Style.WindowPadding.x * 2, label_size.y));



		const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(window->Size.x - g.Style.WindowPadding.x * 2, label_size.y + 20));
		const ImRect frame_bb(window->DC.CursorPos, total_bb.Max - ImVec2(label_size.x + 16, 0));
		...
		window->DrawList->AddLine({ frame_bb.Min.x, frame_bb.Min.y + label_size.y / 2 + 15 }, { grab_bb.Min.x - 2, frame_bb.Min.y + label_size.y / 2 + 15 }, ImColor(210, 115, 138), 2);
		window->DrawList->AddLine({ grab_bb.Max.x + 2, frame_bb.Min.y + label_size.y / 2 + 15 }, { frame_bb.Max.x, frame_bb.Min.y + label_size.y / 2 + 15 }, ImColor(30, 30, 30), 2);
		window->DrawList->AddCircle({ grab_bb.Min.x + g.Style.GrabMinSize / 2, total_bb.Min.y + label_size.y / 2 + 15 }, 6, ImColor(84, 83, 89));


		window->DrawList->AddText(ImVec2(frame_bb.Min.x + 258, frame_bb.Min.y), ImColor(255, 255, 255), value_buf);

		//if (hovered || IsItemActive())
		//{
		//	//PushFont(montherratRegular_10px);
		//	PushStyleColor(ImGuiCol_Text, GetColorU32(ImGuiCol_Text, 0.3f));
		//	RenderText({ grab_bb.Min.x + (grab_bb.Max.x - grab_bb.Min.x) / 2 - CalcTextSize(value_buf).x / 2, total_bb.Max.y - 15 }, value_buf);
		//	PopStyleColor();
		//	//PopFont();
		//	window->DrawList->AddLine({ grab_bb.Min.x - 6, grab_bb.Max.y }, { grab_bb.Max.x + 6, grab_bb.Max.y }, ImColor(77 / 255.f, 78 / 255.f, 82 / 255.f, itemHovered));
		//}
	...
	bool BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
	{
		...
		bool popup_open = ImGui::IsPopupOpen(id, 0);

		const ImU32 frame_col = ImColor(55, 65, 81);
		const float value_x2 = ImMax(frame_bb.Min.x, frame_bb.Max.x - arrow_size);
		ImGui::RenderNavHighlight(frame_bb, id);

		if (!(flags & ImGuiComboFlags_NoArrowButton))
		{
			ImU32 bg_col = ImColor(55, 65, 81);
		...
		ImGui::RenderFrame(total_bb.Min, total_bb.Max, ImColor(30, 30, 30), false, 5);
		...
		ImGui::PushFont(Celestial::Fonts::SmallPoppins);

		if (label_size.x > 0.0f)
			window->DrawList->AddText(ImVec2(total_bb.Min.x + 10, total_bb.Min.y + 15 - ImGui::CalcTextSize(label).y / 2), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 200 / 255.f), label);

		if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
			window->DrawList->AddText(ImVec2(total_bb.Max.x - 20 - ImGui::CalcTextSize(preview_value).x, total_bb.Min.y + 15 - ImGui::CalcTextSize(preview_value).y / 2), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 200 / 255.f), preview_value);
...
void Celestial::Framework::CreateMainWindow(std::string WindowName, std::string Desc)
{
	ImGui::SetNextWindowSize({ 324,459 });

	ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 9.0f);
	ImGui::PushStyleVar(ImGuiStyleVar_WindowBorderSize, 0.0f);
	ImGui::PushStyleColor(ImGuiCol_WindowBg, { 20 / 255.f, 20 / 255.f, 20 / 255.f, 255 / 255.f });


	ImGui::PushFont(Fonts::TitleFont);
	ImGui::Begin(WindowName.c_str(), 0, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize);
	ImGui::GetWindowDrawList()->AddText(ImVec2(ImGui::GetWindowPos().x + 18, ImGui::GetWindowPos().y + 17), ImColor(255, 255, 255), WindowName.c_str());
	ImGui::PopFont();

	ImGui::PushFont(Fonts::SmallTitle);
	ImGui::GetWindowDrawList()->AddText(ImVec2(ImGui::GetWindowPos().x + 18, ImGui::GetWindowPos().y + 36), ImColor(66, 66, 66), Desc.c_str());
	ImGui::PopFont();

	//ImGui::PushFont(Fonts::IconFont);
	//ImGui::GetWindowDrawList()->AddText(ImVec2(ImGui::GetWindowPos().x + 165, ImGui::GetWindowPos().y + 26), ImColor(210, 115, 138), ICON_FA_EYE_LOW_VISION);
	//ImGui::GetWindowDrawList()->AddText(ImVec2(ImGui::GetWindowPos().x + 188, ImGui::GetWindowPos().y + 26), ImColor(66, 66, 66), ICON_FA_BULLSEYE);
	//ImGui::GetWindowDrawList()->AddText(ImVec2(ImGui::GetWindowPos().x + 208, ImGui::GetWindowPos().y + 26), ImColor(66, 66, 66), ICON_FA_GEARS);
	//ImGui::PopFont();
...
bool Celestial::Framework::CreateTab(const char* label, const char* icon, const bool selected, int space, const ImVec2& size_arg)
{
	ImGuiWindow* window = ImGui::GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImRect bb(pos, { pos.x - 25 + size.x , pos.y - 20 + size.y });

	ImGui::ItemSize(ImVec2(size.x - space, size.y), 0);
	...
	{
		float t_anim = ImSaturate(g.LastActiveIdTimer / 0.50f);
		t = (t_anim);
	}

	ImU32 animated_col;

	animated_col = ImGui::GetColorU32(ImLerp(ImColor(66, 66, 66), ImVec4(210 / 255.f, 115 / 255.f, 138 / 255.f, 1.f), t));

	ImGui::PushFont(Fonts::IconFont);

	if (selected)
		window->DrawList->AddText({ bb.Min.x + 10,bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(icon).y / 2 + 1.5f }, animated_col, icon);
	else
		window->DrawList->AddText({ bb.Min.x + 10,bb.Min.y + size_arg.y / 2 - ImGui::CalcTextSize(icon).y / 2 + 1.5f }, animated_col, icon);
...
void Celestial::Framework::BeginTabChild(std::string Title)
{
	ImGui::SetCursorPosX(18);
	ImGui::BeginChild(Title.c_str(), ImVec2(310, 390));
	ImGui::PushFont(Fonts::SmallPoppins);
}

bool Celestial::Framework::Checkmark(const char* label, const char* Desc, bool* v)
{
	ImGuiWindow* window = ImGui::GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	float width = height * 2.30f;
	float radius = height * 0.44f;
	float radius2 = height * 0.53f;

	const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x + 206), label_size.y - 5 + 25));
	ImGui::ItemSize(total_bb, style.FramePadding.y);
	...
	ImGui::PushFont(Fonts::CheckFont);
	ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
	ImGui::RenderText(ImVec2((pos.x + width) - 42, (pos.y + height) - 18), label);
	...
	animated_col = ImGui::GetColorU32(ImLerp(ImColor(30, 30, 30), ImVec4(210 / 255.f, 115 / 255.f, 138 / 255.f, 1.f), t));
	animated_col2 = ImGui::GetColorU32(ImLerp(ImVec4(30 / 255.f, 30 / 255.f, 30 / 255.f, 1.f), ImColor(30, 30, 30), t));
	...
	if (*v)
	{
		...
		ImGui::PushFont(Fonts::IconFont);
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 260, pos.y + 8), animated_col, ICON_FA_SQUARE_CHECK);
		ImGui::PopFont();
	}
	...
	if (!*v)
	{
		const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
		ImGui::PushFont(Fonts::IconFont);
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 260, pos.y + 8), animated_col2, ICON_FA_SQUARE);
		ImGui::PopFont();
	}


	ImGui::PushFont(Fonts::SmallPoppins);
	ImGui::PushStyleColor(ImGuiCol_Text, { 66 / 255.f, 66 / 255.f, 66 / 255.f, 1.f });
	ImGui::RenderText(ImVec2((pos.x + width) - 42, (pos.y + height) - 5), Desc);
	ImGui::PopStyleColor(1);
	ImGui::PopFont();
...
void Celestial::Framework::IntSlider(const char* label, int* v, int v_min, int v_max)
{
	ImGui::PushFont(Fonts::SmallPoppins);
	ImGuiInternals::SliderInt(label, v, v_min, v_max, 0, 0);
	ImGui::PopFont();
}

void Celestial::Framework::MainCombo(std::string Title, int* current_item, const char* items)
{
	ImGuiInternals::Combo(Title.c_str(), current_item, items, 0);
}

void Celestial::Framework::CreateSeperator(std::string Title)
{
	ImGuiWindow* window = ImGui::GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	else if (flags & ImGuiSeparatorFlags_Horizontal)
	{
		...
		{
			// Draw
			window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x - 25, bb.Min.y), ImGui::GetColorU32(ImGuiCol_Separator));
			ImGui::Spacing();
			ImGui::Spacing();
			ImGui::PushFont(Fonts::SmallPoppins);
			ImGui::Spacing();
			window->DrawList->AddText(ImVec2(bb.Min.x, bb.Min.y + 3), ImColor(66, 66, 66), Title.c_str());
			ImGui::PopFont();
			ImGui::Spacing();
			if (g.LogEnabled)
...
void Celestial::Framework::EndTabChild()
{
	ImGui::PopFont();
	ImGui::EndChild();
}

void Celestial::Framework::EndWindow()
{
	ImGui::End();
}
