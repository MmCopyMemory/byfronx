...
#include <Config System/UIManager.h>
#include <mutex>
#include <filesystem>
...
std::once_flag once;
Celestial::Framework* UI;
static int TopTabRowIndex = 0;
char config_name[100];
bool Firs = true;

void Celestial::GUI::Load()
{

	UI->CreateMainWindow("CELESTIALPriv", "made with precision.");
    ImGui::SetCursorPosX(152);
    ImGui::SetCursorPosY(5);

    if (UI->CreateTab("0", ICON_FA_EYE_LOW_VISION, TopTabRowIndex == 0, 25, ImVec2(50, 50)))
    {
        TopTabRowIndex = 0;
    }
    ...
    if (UI->CreateTab("1", ICON_FA_BULLSEYE, TopTabRowIndex == 1, 28, ImVec2(50, 50)))
    {
        TopTabRowIndex = 1;
    }
    ...
    if (UI->CreateTab("2", ICON_FA_GEARS, TopTabRowIndex == 2, 25, ImVec2(50, 50)))
    {
        TopTabRowIndex = 2;
    }
    ...
    if (TopTabRowIndex == 0)
    {
        UI->BeginTabChild("Boom");

        UI->Checkmark("Enable Visuals", "enables all visuals", &Celestial::Globals::RenderMaster);
        UI->Checkmark("Show FOV", "", &Celestial::Globals::FOV);
		if(Celestial::Globals::FOV)
            UI->IntSlider("Value", &Celestial::Globals::FOVValue, 0, 1000);
		
        if(Globals::Game == "Universal" || Globals::Game == "th3ltgrounds" || Globals::Game == "future")
            UI->Checkmark("Override Custom Colors", "uses game team colors", &Celestial::Globals::useteam);

        UI->Checkmark("Show Players", "draws boxes onto players", &Celestial::Globals::PlayerESP);
        UI->Checkmark("Visualize Prediction", "draws a line where it thinks a player will be", &Celestial::Globals::visualize_prediction);
        UI->Checkmark("Show Tracers", "draws tracer lines onto players", &Celestial::Globals::Tracers);
        UI->Checkmark("Show Player Names", "", &Celestial::Globals::NameESP);
        UI->Checkmark("Show Player Misc Info", "shows info like health and distance", &Celestial::Globals::ShowDistance);
       
        if (Globals::Game == "CBRO" || Globals::Game == "Recoil")
        {

        } else { UI->Checkmark("Show Player Skeleton", "draws lines to each body part", &Celestial::Globals::SkeletonESP); }
		
        UI->IntSlider("Max Render Distance", &Celestial::Globals::RenderDist, 0, 10000);

        if (Celestial::Globals::Game == "Bad Business")
        {
            UI->CreateSeperator("Bad Business");
            UI->Checkmark("Show Throwables", "will label thrown items", &Celestial::Globals::ShowGrenades);
            UI->Checkmark("Throwable Warning", "will give onscreen warning when near any thrown item", &Celestial::Globals::GrenadeWarning);
        }

        if (Celestial::Globals::Game == "Phantom Forces")
        {
            UI->CreateSeperator("Phantom Forces");
            UI->Checkmark("Show Guns", "will label dropped guns", &Celestial::Globals::ShowGunNames);
        }

        if (Celestial::Globals::Game == "Apoc 2")
        {
            UI->CreateSeperator("Apocalypse Rising 2");
            UI->Checkmark("Show Locations", "will label locations", &Celestial::Globals::ShowLocations);
            UI->Checkmark("Show Held Items", "will label items that players have equipped", &Celestial::Globals::ShowEquipped);
			
            if (Globals::ShowLocations)
            {
                UI->Checkmark("Location Outline", "", &Celestial::Globals::ShowLocationsOutline);
                UI->IntSlider("Location Distance", &Celestial::Globals::LocationDist, 0, 10000);
                UI->CreateSeperator("");
            }

            UI->Checkmark("Show Vehicles", "will label vehicles", &Celestial::Globals::ShowVehicles);

            if (Globals::ShowVehicles)
            {
                UI->Checkmark("Vehicle Outline", "", &Celestial::Globals::VehicleDistOutline);
                UI->IntSlider("Vehicle Distance", &Celestial::Globals::VehicleDist, 0, 10000);
            }
        }

        if (Celestial::Globals::Game == "ProjDelta")
        {
            UI->CreateSeperator("Project Delta");

            UI->Checkmark("Show Held Guns", "will label guns that players have equipped", &Celestial::Globals::ShowEquippedDELTA);
            if(Celestial::Globals::ShowEquippedDELTA)
                UI->IntSlider("Item Distance", &Celestial::Globals::ItemDist, 0, 10000);

            UI->Checkmark("Show Containers", "will label containers if they contain anything", &Celestial::Globals::ShowContainers);
            if(Celestial::Globals::ShowContainers)
                UI->IntSlider("Container Distance", &Celestial::Globals::ContainerDist, 0, 10000);
        }

        if (Celestial::Globals::Game == "Criminality")
        {
            UI->CreateSeperator("Criminality");
            UI->Checkmark("Show Dealers", "will label dealers", &Celestial::Globals::ShowDealer);

            if (Globals::ShowDealer)
            {
                UI->Checkmark("Dealer Outline", "", &Celestial::Globals::DealerDistOutline);
                UI->IntSlider("Dealer Distance", &Celestial::Globals::DealerDist, 0, 10000);
                UI->CreateSeperator("");
            }

            UI->Checkmark("Show ATMz", "will label atmz", &Celestial::Globals::ShowATM);

            if (Globals::ShowATM)
            {
                UI->Checkmark("ATMz Outline", "", &Celestial::Globals::ATMDistOutline);
                UI->IntSlider("ATMz Distance", &Celestial::Globals::ATMDist, 0, 10000);
            }
        }
    ...
    if (TopTabRowIndex == 1)
    {
        UI->BeginTabChild("Boomy");

        UI->Checkmark("Enable Aimbot", "enables all aimbot options", &Celestial::Globals::AimMaster);
        UI->Checkmark("Prediction", "", &Celestial::Globals::prediction);
        if(Celestial::Globals::prediction)
            UI->IntSlider("Strength", &Celestial::Globals::p_strength, 0, 10);

        UI->IntSlider("Smoothness", &Celestial::Globals::Smoothness, 0, 100);

        ImGui::Combo("Aimbot Key", &Globals::AimKey, aimbot, IM_ARRAYSIZE(aimbot));
        if(Globals::Game != "Riotfall")
            ImGui::Combo("Aim Part", &Globals::AimPart, "Head\0HumanoidRootPart\0");

        UI->IntSlider("X Offset", &Celestial::Globals::XOffset, -50, 50);
        UI->IntSlider("Y Offset", &Celestial::Globals::YOffset, -50, 50);
        UI->IntSlider("Max Lock Distance", &Celestial::Globals::LockDist, 0, 10000);
    ...
    if (TopTabRowIndex == 2)
    {
        UI->BeginTabChild("yuh");

        if (Globals::Game == "Universal" || Globals::Game == "Riotfall" || Globals::Game == "CBRO")
        {
            UI->Checkmark("Teamcheck", "", &Celestial::Globals::Teamcheck);
            UI->Checkmark("Healthcheck", "", &Celestial::Globals::Healthcheck);
        }

        if (Globals::Game == "Rush Point" || Globals::Game == "th3ltgrounds" || Globals::Game == "future" || Globals::Game == "Recoil")
        {
            UI->Checkmark("Teamcheck", "", &Celestial::Globals::Teamcheck);
        }

        UI->CreateSeperator("Configs");
        ...
        if (Firs)
        {
            cConfig CFG_(AppDataCelestial + "\\" + std::string("HOLDER"), CONFIG_READWRITE);
            CFG_.closeOutputStream();
        ...
        for (auto file : std::filesystem::directory_iterator(AppDataCelestial))
        {
            if (!std::filesystem::is_directory(file.path())) {
                auto path2 = file.path().string().substr(AppDataCelestial.length() + 1);
                ret.push_back(path2.c_str());
            }
        }

        ImGui::Text("Config Count: %i", ret.size() - 1);

        static std::string current_item = ret[0];
        if (ImGui::BeginCombo("Configs", current_item.c_str()))
        {
        ...
        ImGui::InputText("Name", config_name, IM_ARRAYSIZE(config_name));

        if (ImGui::Button("Load Config", ImVec2(100, 20)))
            LoadConfig(std::string(current_item).c_str());

        ImGui::SameLine();

        if (ImGui::Button("Save Config", ImVec2(100, 20)))
            SaveConfig(config_name);

        if (ImGui::Button("Open Config Dir", ImVec2(100, 20)))
        {
            ShellExecuteA(NULL, NULL, std::string(std::string(dest) + "\\Celestial\\Configs").c_str(), NULL, NULL, SW_SHOWNORMAL);
        }

        UI->CreateSeperator("Customization");

        ImGui::Combo("Menu Key", &Globals::MenuKey, menu, IM_ARRAYSIZE(menu));

        ImGui::ColorEdit4("Box Color", (float*)&Globals::BoxColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
        ImGui::ColorEdit4("Name Color", (float*)&Globals::NameColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
        ImGui::ColorEdit4("Distance Color", (float*)&Globals::DistanceColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
        ImGui::ColorEdit4("FOV Color", (float*)&Globals::FOVColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
        ImGui::ColorEdit4("Skeleton Color", (float*)&Globals::SkeletonColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
        ImGui::ColorEdit4("Tracer Color", (float*)&Globals::TracersColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);

        if (Celestial::Globals::Game == "Criminality")
        {
            UI->CreateSeperator("Criminality");
            ImGui::ColorEdit4("Dealer Color", (float*)&Globals::DealerColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
            ImGui::ColorEdit4("ATMz Color", (float*)&Globals::ATMColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
        }

        if (Celestial::Globals::Game == "Apoc 2")
        {
            UI->CreateSeperator("Apocalypse Rising 2");
            ImGui::ColorEdit4("Location Color", (float*)&Globals::LocationColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
            ImGui::ColorEdit4("Vehicle Color", (float*)&Globals::VehiclesColor, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoOptions);
        }

        UI->CreateSeperator("Misc");

        UI->IntSlider("Max FPS", &Celestial::Globals::FPSLimit, 60, 1000);
