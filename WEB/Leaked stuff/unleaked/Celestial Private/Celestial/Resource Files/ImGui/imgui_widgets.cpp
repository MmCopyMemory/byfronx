...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration(XorStr( "[" ), XorStrAlloc( "]" ));
...
{
    const char* x = XorStr( "#SCROLLX" );
    const char* y = XorStr( "#SCROLLY" );
    return window->GetID(axis == ImGuiAxis_X ? x : y);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const char* randomthing1 = XorStr( "[~]" );
    const char* randomthing2 = XorStr( "[x]" );
    const char* randomthing3 = XorStr("[ ]" );

    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);
    if (g.LogEnabled)
        LogRenderedText(&label_pos, mixed_value ? randomthing1 : *v ? randomthing2 : randomthing3);
    if (label_size.x > 0.0f)
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    const char* thing1 = XorStr( "(x)" );
    const char* thing2 = XorStr( "( )" );
    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);
    if (g.LogEnabled)
        LogRenderedText( &label_pos, active ? thing1 : thing2 );
    if (label_size.x > 0.0f)
        RenderText(label_pos, label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held);
    const ImGuiID popup_id = ImHashStr(XorStr( "##ComboPopup" ), 0, id);
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    char name[16];
    ImFormatString(name, IM_ARRAYSIZE(name), XorStr( "##Combo_%02d" ), g.BeginPopupStack.Size); // Recycle windows based on depth
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        const char* unknown_item_str = XorStr( "*Unknown item*" );
        if (!items_getter(data, i, &item_text))
            item_text = unknown_item_str;
...
{
    { sizeof(char),             XorStrAlloc( "S8" ),   XorStrAlloc( "%d" ),   XorStrAlloc( "%d" )    },  // ImGuiDataType_S8
    { sizeof(unsigned char),    XorStrAlloc( "U8" ),   XorStrAlloc( "%u" ),   XorStrAlloc( "%u" )    },
    { sizeof(short),            XorStrAlloc( "S16" ),  XorStrAlloc( "%d" ),   XorStrAlloc( "%d" )    },  // ImGuiDataType_S16
    { sizeof(unsigned short),   XorStrAlloc( "U16" ),  XorStrAlloc( "%u" ),   XorStrAlloc( "%u" )    },
    { sizeof(int),              XorStrAlloc( "S32" ),  XorStrAlloc( "%d" ),   XorStrAlloc( "%d" )    },  // ImGuiDataType_S32
    { sizeof(unsigned int),     XorStrAlloc( "U32" ),  XorStrAlloc( "%u" ),   XorStrAlloc( "%u" )    },
#ifdef _MSC_VER
    { sizeof(ImS64),            XorStrAlloc( "S64" ),  XorStrAlloc( "%I64d" ), XorStrAlloc( "%I64d" ) },  // ImGuiDataType_S64
    { sizeof(ImU64),            XorStrAlloc( "U64" ),  XorStrAlloc( "%I64u" ), XorStrAlloc( "%I64u" ) },
#else
    { sizeof(ImS64),            XorStrAlloc( "S64" ),  XorStrAlloc( "%lld" ), XorStrAlloc( "%lld" )  },  // ImGuiDataType_S64
    { sizeof(ImU64),            XorStrAlloc( "U64" ),  XorStrAlloc( "%llu" ), XorStrAlloc( "%llu" )  },
#endif
    { sizeof(float),            XorStrAlloc( "float" ), XorStrAlloc( "%.3f" ), XorStrAlloc( "%f" )    },  // ImGuiDataType_Float (float are promoted to double in va_arg)
    { sizeof(double),           XorStrAlloc( "double" ), XorStrAlloc( "%f" ),  XorStrAlloc( "%lf" )   },  // ImGuiDataType_Double
};
...
bool ImGui::DragFloatRange2(const char* label, float* v_current_min, float* v_current_max, float v_speed, float v_min, float v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    bool value_changed = DragScalar(XorStr( "##min" ), ImGuiDataType_Float, v_current_min, v_speed, &min_min, &min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    value_changed |= DragScalar(XorStr( "##max" ), ImGuiDataType_Float, v_current_max, v_speed, &max_min, &max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
bool ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max, float v_speed, int v_min, int v_max, const char* format, const char* format_max, ImGuiSliderFlags flags)
{
    ...
    ImGuiSliderFlags min_flags = flags | ((min_min == min_max) ? ImGuiSliderFlags_ReadOnly : 0);
    bool value_changed = DragInt(XorStr( "##min" ), v_current_min, v_speed, min_min, min_max, format, min_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
    ...
    ImGuiSliderFlags max_flags = flags | ((max_min == max_max) ? ImGuiSliderFlags_ReadOnly : 0);
    value_changed |= DragInt(XorStr( "##max" ), v_current_max, v_speed, max_min, max_max, format_max ? format_max : format, max_flags);
    PopItemWidth();
    SameLine(0, g.Style.ItemInnerSpacing.x);
...
{
    const char* randomstuff = XorStr( "%.0f deg" );
    if (format == NULL)
        format = randomstuff;
...
{
    const char* fmt1 = XorStr( "%08X" );
    const char* fmt2 = XorStr( "%d" );

    // Hexadecimal input provided as a convenience but the flag name is awkward. Typically you'd use InputText() to parse your own data, if you want to handle prefixes.
    const char* format = (flags & ImGuiInputTextFlags_CharsHexadecimal) ? fmt1 : fmt2;
    return InputScalar(label, ImGuiDataType_S32, (void*)v, (void*)(step > 0 ? &step : NULL), (void*)(step_fast > 0 ? &step_fast : NULL), format, flags);
}

bool ImGui::InputInt2(const char* label, int v[2], ImGuiInputTextFlags flags)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 2, NULL, NULL, XorStr( "%d" ), flags);
}

bool ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags flags)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 3, NULL, NULL, XorStr( "%d" ), flags);
}

bool ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags flags)
{
    return InputScalarN(label, ImGuiDataType_S32, v, 4, NULL, NULL, XorStr( "%d" ), flags);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick(XorStr( "context" ), ImGuiPopupFlags_MouseButtonRight);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ColorButton(XorStr( "##current" ), col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text(XorStr( "Original" ));
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            if (ColorButton(XorStr( "##original" ), ref_col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2)))
            {
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            if (ColorEdit4(XorStr( "##rgb" ), col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(XorStr( "##hsv" ), col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4(XorStr( "##hex" ), col, sub_flags | ImGuiColorEditFlags_DisplayHex);
        PopItemWidth();
    }
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ColorButton(XorStr( "##preview" ), cf, (flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
    SameLine();
    ...
    {
        if (flags & ImGuiColorEditFlags_NoAlpha)
            Text(XorStr( "H: %.3f, S: %.3f, V: %.3f" ), col[0], col[1], col[2]);
        else
            Text(XorStr( "H: %.3f, S: %.3f, V: %.3f, A: %.3f" ), col[0], col[1], col[2], col[3]);
    }
    EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_inputs)
    {
        if (RadioButton(XorStr( "RGB" ), (opts & ImGuiColorEditFlags_DisplayRGB) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayRGB;
        if (RadioButton(XorStr( "HSV" ), (opts & ImGuiColorEditFlags_DisplayHSV) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHSV;
        if (RadioButton(XorStr( "Hex" ), (opts & ImGuiColorEditFlags_DisplayHex) != 0)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHex;
    }
    if (allow_opt_datatype)
    {
        if (allow_opt_inputs) Separator();
        if (RadioButton(XorStr( "0..255" ),     (opts & ImGuiColorEditFlags_Uint8) != 0)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Uint8;
        if (RadioButton(XorStr( "0.00..1.00" ), (opts & ImGuiColorEditFlags_Float) != 0)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Float;
    }
    ...
    if (BeginPopup(XorStr( "Copy" )))
    {
        ...
        if (Selectable(buf))
            SetClipboardText(buf);
        ImFormatString(buf, IM_ARRAYSIZE(buf), XorStr( "#%02X%02X%02X" ), cr, cg, cb);
        if (Selectable(buf))
            SetClipboardText(buf);
        if (!(flags & ImGuiColorEditFlags_NoAlpha))
        {
            ImFormatString(buf, IM_ARRAYSIZE(buf), XorStr( "#%02X%02X%02X%02X" ), cr, cg, cb, ca);
            if (Selectable(buf))
                SetClipboardText(buf);
        }
        EndPopup();
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if ((!allow_opt_picker && !allow_opt_alpha_bar) || !BeginPopup(XorStr( "context" )))
        return;
    ImGuiContext& g = *GImGui;
    if (allow_opt_picker)
    {
        ...
        for (int picker_type = 0; picker_type < 2; picker_type++)
        {
            ...
            if (Selectable(XorStr( "##selectable" ), false, 0, picker_size)) // By default, Selectable() is closing popup
                g.ColorEditOptions = (g.ColorEditOptions & ~ImGuiColorEditFlags_PickerMask_) | (picker_flags & ImGuiColorEditFlags_PickerMask_);
            SetCursorScreenPos(backup_pos);
            ImVec4 previewing_ref_col;
            memcpy(&previewing_ref_col, ref_col, sizeof(float) * ((picker_flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4));
            ColorPicker4(XorStr( "##previewing_picker" ), &previewing_ref_col.x, picker_flags);
            PopID();
        }
        PopItemWidth();
    }
    if (allow_opt_alpha_bar)
    {
        ...
        CheckboxFlags(XorStr( "Alpha Bar" ), &g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar);
    }
    EndPopup();
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    else
    {
        ...
        if (g.LogEnabled)
            LogSetNextTextDecoration(XorStr( ">" ), NULL);
...
bool ImGui::CollapsingHeader(const char* label, bool* p_visible, ImGuiTreeNodeFlags flags)
{
    ...
    if (p_visible != NULL)
    {
        ...
        ImGuiID close_button_id = GetIDWithSeed(XorStr( "#CLOSE" ), NULL, id);
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    while (clipper.Step())
        ...
        {
            const char* item_text;
            const char* unknown_str = XorStr( "*Unknown item*" );
            if (!items_getter(data, i, &item_text))
                item_text = unknown_str;
...
{
    const char* str_true = XorStr( "true" );
    const char* str_false = XorStr( "false" );

    Text( XorStr( "%s: %s" ), prefix, ( b ? str_true : str_false ) );
}
...
bool ImGui::BeginMainMenuBar()
{
    ...
    bool is_open = BeginViewportSideBar(XorStr( "##MainMenuBar" ), viewport, ImGuiDir_Up, height, window_flags);
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
    ...
    if (ArrowButtonEx(XorStr( "##<" ), ImGuiDir_Left, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
        select_dir = -1;
    window->DC.CursorPos = ImVec2(x + arrow_button_size.x, tab_bar->BarRect.Min.y);
    if (ArrowButtonEx(XorStr( "##>" ), ImGuiDir_Right, arrow_button_size, ImGuiButtonFlags_PressedOnClick | ImGuiButtonFlags_Repeat))
        select_dir = +1;
    PopStyleColor(2);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo(XorStr( "##v" ), NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    const ImGuiID close_button_id = p_open ? GetIDWithSeed(XorStr( "#CLOSE" ), NULL, id) : 0;
    bool just_closed;
    bool text_clipped;
    ...
    if (text_clipped && g.HoveredId == id && !held)
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip) && !(tab->Flags & ImGuiTabItemFlags_NoTooltip))
            if (IsItemHovered(ImGuiHoveredFlags_DelayNormal))
                SetTooltip(XorStr( "%.*s" ), (int)(FindRenderedTextEnd(label) - label), label);
