...
void ImGui::TableHeader(const char* label)
{
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        ...
        {
            ImFormatString(sort_order_suf, IM_ARRAYSIZE(sort_order_suf), XorStr( "%d" ), column->SortOrder + 1);
    ...
    if (text_clipped && hovered && g.ActiveId == 0 && IsItemHovered(ImGuiHoveredFlags_DelayNormal))
        SetTooltip(XorStr( "%.*s" ), (int)(label_end - label), label);
...
void ImGui::TableOpenContextMenu(int column_n)
{
    ...
    if (table->Flags & (ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable))
    {
        ...
        table->InstanceInteracted = table->InstanceCurrent;
        const ImGuiID context_menu_id = ImHashStr(XorStr( "##ContextMenu" ), 0, table->ID);
...
void ImGui::TableDrawContextMenu(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_Resizable)
    {
        if (column != NULL)
        {
            ...
            if (MenuItem(XorStr( "Size column to fit###SizeOne" ), NULL, false, can_resize))
                TableSetColumnWidthAutoSingle(table, column_n);
        }

        const char* first_option = XorStr( "Size all columns to fit###SizeAll" );
        const char* second_option = XorStr( "Size all columns to default###SizeAll" );

        const char* size_all_desc;
        if (table->ColumnsEnabledFixedCount == table->ColumnsEnabledCount && (table->Flags & ImGuiTableFlags_SizingMask_) != ImGuiTableFlags_SizingFixedSame)
            size_all_desc = first_option;        // All fixed
        else
            size_all_desc = second_option;    // All stretch or mixed
    ...
    {
        if (MenuItem(XorStr( "Reset order" ), NULL, false, !table->IsDefaultDisplayOrder))
            table->IsResetDisplayOrderRequest = true;
        want_separator = true;
    }
    ...
    if (table->Flags & ImGuiTableFlags_Hideable)
    {
        ...
        for (int other_column_n = 0; other_column_n < table->ColumnsCount; other_column_n++)
        {
            ...
            const char* str_unknown = XorStr( "<Unknown>" );
...
static void* TableSettingsHandler_ReadOpen(ImGuiContext*, ImGuiSettingsHandler*, const char* name)
{
    ImGuiID id = 0;
    int columns_count = 0;
    if (sscanf(name, XorStr( "0x%08X,%d" ), &id, &columns_count) < 2)
        return NULL;
...
static void TableSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line)
{
    ...
    if (sscanf(line, XorStr( "RefScale=%f" ), &f) == 1) { settings->RefScale = f; return; }

    if (sscanf(line, XorStr( "Column %d%n" ), &column_n, &r) == 1)
    {
        ...
        if (sscanf(line, XorStr( "UserID=0x%08X%n" ), (ImU32*)&n, &r)==1) { line = ImStrSkipBlank(line + r); column->UserID = (ImGuiID)n; }
        if (sscanf(line, XorStr( "Width=%d%n" ), &n, &r) == 1)            { line = ImStrSkipBlank(line + r); column->WidthOrWeight = (float)n; column->IsStretch = 0; settings->SaveFlags |= ImGuiTableFlags_Resizable; }
        if (sscanf(line, XorStr( "Weight=%f%n" ), &f, &r) == 1)           { line = ImStrSkipBlank(line + r); column->WidthOrWeight = f; column->IsStretch = 1; settings->SaveFlags |= ImGuiTableFlags_Resizable; }
        if (sscanf(line, XorStr( "Visible=%d%n" ), &n, &r) == 1)          { line = ImStrSkipBlank(line + r); column->IsEnabled = (ImU8)n; settings->SaveFlags |= ImGuiTableFlags_Hideable; }
        if (sscanf(line, XorStr( "Order=%d%n" ), &n, &r) == 1)            { line = ImStrSkipBlank(line + r); column->DisplayOrder = (ImGuiTableColumnIdx)n; settings->SaveFlags |= ImGuiTableFlags_Reorderable; }
        if (sscanf(line, XorStr( "Sort=%d%c%n" ), &n, &c, &r) == 2)       { line = ImStrSkipBlank(line + r); column->SortOrder = (ImGuiTableColumnIdx)n; column->SortDirection = (c == '^') ? ImGuiSortDirection_Descending : ImGuiSortDirection_Ascending; settings->SaveFlags |= ImGuiTableFlags_Sortable; }
    }
}
...
static void TableSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    for (ImGuiTableSettings* settings = g.SettingsTables.begin(); settings != NULL; settings = g.SettingsTables.next_chunk(settings))
    {
        ...
        buf->appendf(XorStr( "[%s][0x%08X,%d]\n" ), handler->TypeName, settings->ID, settings->ColumnsCount);
        if (settings->RefScale != 0.0f)
            buf->appendf(XorStr( "RefScale=%g\n" ), settings->RefScale);
        ...
        for (int column_n = 0; column_n < settings->ColumnsCount; column_n++, column++)
        {
            ...
            if (!save_column)
                continue;
            buf->appendf(XorStr( "Column %-2d" ), column_n);
            if (column->UserID != 0)                    buf->appendf(XorStr( " UserID=%08X" ), column->UserID);
            if (save_size && column->IsStretch)         buf->appendf(XorStr( " Weight=%.4f" ), column->WidthOrWeight);
            if (save_size && !column->IsStretch)        buf->appendf(XorStr( " Width=%d" ), (int)column->WidthOrWeight);
            if (save_visible)                           buf->appendf(XorStr( " Visible=%d" ), column->IsEnabled);
            if (save_order)                             buf->appendf(XorStr( " Order=%d" ), column->DisplayOrder);
            if (save_sort && column->SortOrder != -1)   buf->appendf(XorStr( " Sort=%d%c" ), column->SortOrder, (column->SortDirection == ImGuiSortDirection_Ascending) ? 'v' : '^');
            buf->append(XorStr( "\n" ));
        }
        buf->append(XorStr( "\n" ));
    }
...
void ImGui::TableSettingsAddSettingsHandler()
{
    ...
    ini_handler.TypeName = XorStrAlloc( "Table" );
    ini_handler.TypeHash = ImHashStr(XorStr( "Table" ));
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    const char* str_columns = XorStr( "columns" );
    // Differentiate column ID with an arbitrary prefix for cases where users name their columns set the same as another widget.
    // In addition, when an identifier isn't explicitly provided we include the number of columns in the hash to make it uniquer.
    PushID(0x11223347 + (str_id ? 0 : columns_count));
    ImGuiID id = window->GetID( str_id ? str_id : str_columns );
    PopID();
